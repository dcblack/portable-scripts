...
=head1 SYNOPSIS
 ...
 use Envv qw{alias setenv export setlocal source
            shell realpath reduce remove splicenv ignorenvv
...
subroutines in B<Envv> with fairly familiar syntax. These include:
 ...
 use Envv qw{setenv export alias remove splicenv realpath reduce};
 ...
 realpath  'NAME'; # expand a list of paths that may include symlinks to realpath's
...
Replaces a list of file paths to their full pathnames if they are anchored at root (/).
Assumes environmental variable format in scalar context.

 realpath LIST

 Example 1: scalar &realpath('.:~/eda:/link') ==> '.:/eda:/real/dir/path'
...
BEGIN {
    ...
    %EXPORT_TAGS = (
        all   => [ qw(&alias &unalias &setenv &unset &export &setlocal
                      ...
                      &reducepath &realpath &remove &splicenv
                      &ignorenvv &shell $shell &envv &Printf
                      &envvfile &envvopt &expand_path &is_list
                      &envdiff &envsave &envload &envdump &envprepend
                      &envappend &envinsert &envremove &relative_path
                      &find_path &find_dir_containing &dump &getsoc
                      &showdir) ],
        basic => [ qw{&alias &unalias &setenv &unset &export &setlocal
                      &source &prescript &postscript &module &reduce
                      &reducepath &realpath &remove &splicenv
                      &ignorenvv &shell &expand_path &envprepend
                      &envappend &envinsert &envremove &Printf
                      &relative_path &find_path} ],
        diff  => [ qw{&alias &unalias &setenv &unset &export &setlocal
                      &source &prescript &postscript &module &reduce
                      &reducepath &realpath &remove &splicenv
                      &ignorenvv &shell &expand_path &is_list
                      &envdiff &envsave &envload &envdump &envprepend
                      &envappend &envinsert &envremove &Printf
                      &relative_path &find_path} ],
        setup => [ qw{&alias &unalias &setenv &unset &export &setlocal
                      &source &prescript &postscript &module &reduce
                      &reducepath &realpath &remove &splicenv
                      &ignorenvv &shell &expand_path &Printf
    ...
    %ABS = ();   # list variables to be expanded
...
########################################################################
# Similar to reduce, but assumes each element is a real file pathname
# and resolves it before deciding.
sub reducepath(@) { #<<<2
    my (@old) = @_;
    my (%old,@new,$old);
    if (scalar @old == 1 and $old[0] =~ m/^\w+/ and defined $ENV{$old[0]}) {
...
sub realpath(@) { #<<<2
    ...
    foreach $old (@old) {
      if ($old =~ m{^[/~]}) {
        my $new = &abs_path($old);
        push(@new,$new) if defined $new and -e $new;
      } else {
        push(@new,$old);
      }#endif
    }#endforeach
...
sub envv($) { #<<<2
    ...
    #-------------------------------------------------------------------
    # Expand specified variables
    #-------------------------------------------------------------------
    for $var (keys %ABS) {
        printf STDERR "INFO: expanding $var\n" if $verbosity eq 'debug';
        $ENV{$var} = realpath $ENV{$var};
    }#endfor
